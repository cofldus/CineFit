...
const checkboxCls = 'h-5 w-5 accent-ed-gold';
...
export function OnboardingCard() {
  ...
  return (
    <section aria-label="간단한 취향 안내">
      <h2 className="m-0 font-display text-base font-bold tracking-[-0.035em] leading-[1.1] text-ed-ink">
        세 가지만 알려주시면 기본값을 맞춰드려요
      </h2>
      <p className="mt-1 text-sm leading-[1.65] tracking-[-0.015em] text-ed-ink-muted">
        언제든 추천 조건 입력 화면에서 다시 바꿀 수 있어요.
      </p>
      <form onSubmit={handleSubmit} className="mt-3 flex flex-col gap-3.5">
        ...
        <SegmentedControl
          ...
          defaultValue="0"
        />
        <label className="flex min-h-11 items-center gap-2.5 text-[15px] text-ed-ink">
        ...
        <div className="flex gap-2">
          <button
            type="submit"
            className="min-h-11 flex-1 bg-ed-ink px-4 text-sm font-semibold text-ed-canvas transition-opacity hover:opacity-85"
          >
            저장하고 시작
          </button>
          <button
            type="button"
            onClick={handleSkip}
            className="min-h-11 border border-ed-hairline px-4 text-sm font-medium text-ed-ink-muted hover:text-ed-ink"
          >
            건너뛰기
          </button>
        </div>
