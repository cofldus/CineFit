...
//
// 이중 방언: getAppDbClient를 쓰므로 DATABASE_PROVIDER=postgres + 연결 문자열 환경변수로
// 운영 PostgreSQL에도 같은 스크립트를 그대로 실행할 수 있다(멱등 UPSERT).
//   로컬:  node db/seed-movie-specs-2026-summer.mjs
//   운영:  DATABASE_PROVIDER=postgres node --env-file=.env db/seed-movie-specs-2026-summer.mjs
//
// 조사 근거(요약):
...
async function getOrCreateImdbSource() {
  const rows = await db.query(`SELECT id FROM sources WHERE name = 'IMDb'`);
...
const imdbSourceId = await getOrCreateImdbSource();

let inserted = 0;
let skipped = 0;
for (const [kobis, label, key, value, status, confidence] of SPECS) {
  const movie = (await db.query(`SELECT id FROM movies WHERE kobis_code = ?`, [kobis]))[0];
  if (!movie) {
  ...
  await db.run(
    ...
    [movie.id, key, value, imdbSourceId, status, OBSERVED_AT, confidence],
  );
  inserted++;
}

console.log(`영화 사양 시드 완료 — 반영 ${inserted}건, 건너뜀 ${skipped}건`);
console.log(
