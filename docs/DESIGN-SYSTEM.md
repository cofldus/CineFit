...
| MovieCard 화면비 프레임 | `components/MovieCard.tsx` | 포스터 대체 — 영화의 실제 화면비(native_ar)를 프레임 모양 자체로 시각화(장식이 아닌 데이터 표현). 커튼·스크린 일러스트는 폐기하고 `compact`/`detailed` variant가 같은 원칙을 공유 |
