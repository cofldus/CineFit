...
test.describe('시각 회귀', () => {
  ...
  test('추천 조건 입력', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { level: 1 })).toBeVisible();
    // R20: 실시간 후보 상태(디바운스 fetch)가 확정된 뒤에 찍는다 — '계산 중' 상태로 찍히면
    // 타이밍에 따라 스냅샷이 흔들린다. e2e 시드는 합성 회차라 '회차 데이터 연결 전'이 정답.
  ...
  test('개인정보 삭제 요청', async ({ page }) => {
    await page.setViewportSize(VIEWPORTS.mobile);
    await page.goto('/privacy');
    await expect(page.getByRole('heading', { level: 1 })).toBeVisible();
    await expect(page).toHaveScreenshot('privacy-requests.png', { fullPage: true, ...SCREENSHOT_OPTS });
  });
  ...
  test('관리자 예매 링크 상태', async ({ page }) => {
    await page.setViewportSize(VIEWPORTS.desktop);
    await page.goto('/admin/login');
    await page.getByLabel('관리자 비밀번호').fill('e2e-admin-pw');
    await page.getByRole('button', { name: '로그인' }).click();
    await expect(page.getByRole('heading', { name: '관리자 대시보드' })).toBeVisible();
    await page.goto('/admin/booking-links');
