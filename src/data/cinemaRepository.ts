...
const parseJson = (s: string | null): Record<string, unknown> => (s ? (JSON.parse(s) as Record<string, unknown>) : {});
...
export const cinemaRepository = {
  ...
  getAuditoriumDetail(id: number, nowIso: string): AuditoriumDetail | null {
    const db = getDb();
    const aud = db
      .prepare(
    ...
    const specRows = db
      .prepare(
        `SELECT sp.*, s.name AS source_name, s.url AS source_url
         ...
         ORDER BY (sp.valid_to IS NULL) DESC, sp.valid_from DESC`,
      )
      .all(id) as unknown as Record<string, string | number | null>[];
    ...
    return {
      ...
      specHistory,
      observations,
      seatZones: seatZoneRepository.listByAuditorium(id),
      upcomingShowtimes,
    };
  },
};
